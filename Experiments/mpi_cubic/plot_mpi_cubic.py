#!/usr/bin/env python3
"""
Plot results from MPI cubic Poisson solver.

This script automatically finds and plots the most recent data from compute_mpi_cubic.py.
"""
...
def plot_convergence_history(df_global: pd.DataFrame, output_dir: Path):
    """Plot convergence history if available."""
    if "residual_history" in df_global.columns:
        residual_history = df_global["residual_history"].iloc[0]

        fig, ax = plt.subplots(figsize=(10, 6))
        ax.semilogy(range(1, len(residual_history) + 1), residual_history, 'b-', linewidth=2)
        ...
        output_file = output_dir / "convergence_history.pdf"
        plt.savefig(output_file)
        plt.close()
        print(f"Convergence history plot saved to: {output_file}")
...
def print_summary(df_config: pd.DataFrame, df_global: pd.DataFrame, df_perrank: pd.DataFrame):
    ...
    mpi_ranks = df_config["mpi_ranks"].iloc[0]
    ...
    if "residual_history" in df_global.columns:
        residual_history = df_global["residual_history"].iloc[0]
        final_residual = residual_history[-1] if len(residual_history) > 0 else 0.0
...
def main():
    ...
    if not data_dir.exists():
        ...
        print("Run compute_mpi_cubic.py first to generate data")
        return
    ...
    plot_convergence_history(df_global, figures_dir)
